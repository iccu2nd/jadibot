...
export async function loadPlugins() {
    ...
    // Import semua file plugin PARALEL, bukan satu-satu berurutan — dynamic
    // import itu I/O-bound (baca + compile file), jadi aman dijalankan bareng
    // dan ini motong waktu loading dari "total semua file" jadi "selambat
    // file paling lambat".
    const files = getAllFiles(pluginFolder)
    const results = await Promise.allSettled(files.map(async (fullPath) => {
        ...
        return { relativePath, rawPlugin }
    }))

    for (let i = 0; i < results.length; i++) {
        const result = results[i]
        const relativePath = path.relative(pluginFolder, files[i])
        if (result.status === 'rejected') {
            failed++
            errors.push({ file: relativePath, error: (result.reason?.message || String(result.reason)).split('\n')[0] })
            continue
        }
        const { rawPlugin } = result.value
