...
import { getAllCommandEntries, isPremiumActive } from './plugins.js'
...
// Nomor JID yang pernah chat pribadi (bukan grup) ke sesi jadibot ini —
// dipakai buat ngirim ads "follow saluran" ke user free (non-premium) tanpa
// nyimpen histori chat penuh, cukup Set of JID per nomor jadibot, di memori.
...
async function resolveBaileysVersion() {
    ...
    try {
        const { version, isLatest } = await Promise.race([
            fetchLatestBaileysVersion(),
            new Promise((_, rej) => setTimeout(() => rej(new Error('timeout ambil versi Baileys')), 5000))
        ])
...
const ADS_INTERVAL_MS = 8 * 60 * 60 * 1000 // 8 jam sekali → 3x sehari
const ADS_TEXTS = [`Bot ini dibuat pakai *${config.botName}*.`]
const FREE_AD_TEXT = `Halo! Biar nggak ketinggalan update fitur & info terbaru dari *${config.botName}*, yuk follow saluran WhatsApp kami dulu 👇`
...
// Dipanggil tiap ada pesan chat pribadi (bukan grup) masuk ke sesi jadibot,
// supaya nanti bisa dikirimi ads follow-saluran kalau dia bukan owner/premium.
function trackPrivateUser(childSock, jid) {
    if (!jid || jid.endsWith('@g.us')) return
    const selfJid = childSock.user?.id ? jidNormalizedUser(childSock.user.id) : null
    const extraOwnerNumber = childSock.jadibotSettings?.ownerNumber
    const extraOwnerJid = extraOwnerNumber ? extraOwnerNumber.replace(/[^0-9]/g, '') + '@s.whatsapp.net' : null
    if (jid === selfJid || jid === extraOwnerJid) return // owner nggak perlu diiklanin

    const number = childSock.jadibotNumber
    if (!privateChatUsers.has(number)) privateChatUsers.set(number, new Set())
    privateChatUsers.get(number).add(jid)
}
...
// Ads pribadi (DM) ke user FREE aja — yang chat langsung ke jadibot ini dan
// belum premium — ngajak follow saluran WhatsApp. User premium dilewati
// (dianggap udah "pelanggan", nggak perlu diiklanin lagi).
async function sendPrivateAds(number) {
    const entry = activeSessions.get(number)
    if (!entry || entry.status !== 'connected' || !entry.sock) return 0
    const jids = privateChatUsers.get(number)
    if (!jids || !jids.size) return 0

    let sentCount = 0
    for (const jid of jids) {
        const user = global.db?.data?.users?.[jid]
        if (isPremiumActive(user)) continue
        const ok = await entry.sock.sendInteractiveButton(jid, {
            body: FREE_AD_TEXT,
            footer: config.botName,
            buttons: [{ type: 'url', label: '📢 Follow Saluran WhatsApp', url: ADS_CHANNEL_URL }]
        }).then(() => true).catch(() => false)
        if (ok) sentCount++
        await sleep(ADS_SEND_DELAY_MS)
    }
...
async function guardedHandle(childSock, event) {
    ...
    try {
        ...
        if (raw?.message && !raw.key?.fromMe) {
            ...
            trackPrivateUser(childSock, raw.key?.remoteJid)
        }
    } catch (e) {}
...
async function stopSession(number) {
    clearLinkDeadline(number)
    clearAdsTimer(number)
    privateChatUsers.delete(number)
    notifiedConnected.delete(number)
...
async function startChildSession(number, requesterJid = null, mainSock = null) {
    ...
    if (!childSock.authState.creds.registered) {
        try {
            // Delay ini dulu 1500ms/3000ms — buffer kasar biar koneksi WS
            // sempat kebuka sebelum requestPairingCode() dipanggil. Dipangkas
            // ke nilai lebih kecil (masih ada jeda tipis biar nggak race,
            // tapi nggak bikin nunggu lama pas connect).
